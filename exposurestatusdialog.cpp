...
ExposureStatusDialog::ExposureStatusDialog(andor2k::ClientSocket *socket,
                                           int nimages, QWidget *parent)
    : QDialog(parent) {
  ...
  sgbx_layout->addWidget(new QLabel(tr("Last Acquired Temperature in degC:")),
                         0, 0);
  ...
  printf("[DEBUG][ANDOR2K::client::%15s] ExposureStatusDialog Socket at %p",
         __func__, &csock);
  if (csock)
    printf(" -> %p\n", csock);
  else
    printf(" -> nowhere!\n");
  printf("[DEBUG][ANDOR2K::client::%15s] Finished constructing "
         "ExposureStatusDialog\n",
         __func__);
